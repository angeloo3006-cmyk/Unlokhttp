...
impl QueryBuilder {
    fn new() -> Self {
        Self { clauses: Vec::new(), values: Vec::new() }
    }
    ...
    {
        self.clauses.push(format!("{column} = ?{}", self.values.len() + 1));
    ...
    /// Push a `field LIKE ?` clause (caller adds `%` wildcards as needed).
    fn like<V>(&mut self, column: &str, value: V)
    where
        V: rusqlite::types::ToSql + 'static,
    {
        self.clauses.push(format!("{column} LIKE ?{}", self.values.len() + 1));
        self.values.push(Box::new(value));
    }
...
impl DbManager {
    ...
    pub fn get_protocol_stats(
        &self,
        session_id: i64,
    ) -> Result<Vec<ProtocolStat>, String> {
        let conn = self.lock()?;
        let mut stmt = conn
            .prepare(
    ...
    pub fn build_diagnostics_data(
        ...
        let (total_packets, total_bytes, avg_packet_size) =
    ...
    fn apply_filters(qb: &mut QueryBuilder, f: &PacketFilters) {
        if let Some(ref v) = f.src_ip     { qb.eq("src_ip",   v.clone()); }
        if let Some(ref v) = f.dst_ip     { qb.eq("dst_ip",   v.clone()); }
        if let Some(v)     = f.src_port   { qb.eq("src_port", v as i32); }
        if let Some(v)     = f.dst_port   { qb.eq("dst_port", v as i32); }
        if let Some(ref v) = f.protocol   { qb.eq("protocol", v.clone()); }
        if let Some(v)     = f.min_length { qb.gte("length",  v as i32); }
        if let Some(v)     = f.max_length { qb.lte("length",  v as i32); }
